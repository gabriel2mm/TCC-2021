...
function BasicSelectComponent({ dataSource, defaultOption, onChange, className, props, disabled, value }) {
  ...
  return (
    disabled ? (<BasicInputComponent {...props} value={value} disabled/>) : (<select {...props} value={value} onChange={onChange} className={`${className} ${classBase}`}>
