...
export default function ActivityDetailPage() {
  ...
  const [data, setData] = useState({ activity: "", description: "", assigned: {}, created: new Date(), dateLimit: new Date(), viewFormComplement: false, formComplement: "", tableComplement: [], complementVisible: true });
        ...
        setData(
          {
            ...
            tableComplement: response.data?.complements.map(item => ({ key: item.id, id: item.id, title: item.complement, user: item.createdBy, date: item.created })),
          ...
          <Form initialValues={data} onFinish={handleSubmit} form={form} scrollToFirstError>
            <div className="flex flex-col w-full">
              <div className="flex flex-row w-full justify-end">
                ...
                {data.status === "ABERTO" && (context.containsPermission("Admin") || context.containsPermission("receive:activity") || context.containsPermission("activities")) ? (
                ...
                {data.status === "EM_ANDAMENTO" && (context.containsPermission("Admin") || context.containsPermission("receive:activity") || context.containsPermission("activities")) ? (
              ...
              {context.containsPermission("Admin") || context.containsPermission("activities") ? (
                <div className="flex md:flex-row flex-col w-full">
                  <div className="item-group w-full md:mr-2">
                    ...
                    <Form.Item>
                      <BasicSelectComponent onChange={handleChangeUser} value={data.assigned?.id} dataSource={users.map(user => ({ option: `${user.firstName} ${user.lastName} - ${user.email}`, value: user.id }))} />
                    </Form.Item>
                  </div>
                </div>
              ) : (null)}
            ...
            <Tabs defaultActiveKey="1">
              ...
              {data.status === "CONCLUIDO" || data.status === "Concluido" ? (
            ...
            <Form.Item>
              {context.containsPermission("Admin") || context.containsPermission("activities") ? (<ButtonComponent name="save" type="submit">Atualizar</ButtonComponent>) : null}
